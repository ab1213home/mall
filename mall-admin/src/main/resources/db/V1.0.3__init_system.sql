...
CREATE TABLE `tb_templates`  (
  ...
  `isDel` char(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT '0' COMMENT '是否删除, 逻辑删除标志位',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `unique_channel_purpose`(`channel` ASC, `purpose` ASC) USING BTREE,
  INDEX `idx_name`(`name` ASC) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '模板表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `tb_notice_logs`  (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `templateId` bigint NULL DEFAULT NULL COMMENT '通知模板ID',
  `properties` json NULL COMMENT '通知模板属性',
  `receiver` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '通知接收者',
  `status` int NOT NULL COMMENT '通知状态',
  `triggerTime` datetime NULL DEFAULT CURRENT_TIMESTAMP COMMENT '触发时间',
  `triggerPerson` bigint NULL DEFAULT NULL COMMENT '触发人',
  PRIMARY KEY (`id`) USING BTREE,
