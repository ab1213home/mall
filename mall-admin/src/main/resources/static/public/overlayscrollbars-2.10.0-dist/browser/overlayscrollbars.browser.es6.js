...
var OverlayScrollbarsGlobal = function(t) {
  ...
  const createDOMObserver = (t, n, o, s) => {
    let e = false;
    const {F: c, Y: r, W: l, X: i, G: a, J: u} = s || {};
  ...
  const createStructureSetupElements = t => {
    ...
    const Y = {
      ...
      Ot: () => addRemoveAttrClass(B, j, X, true)
    };
    const {vt: W, ht: J, ln: Z, ot: tt, bt: nt} = Y;
    const ot = [ () => {
      removeAttrs(J, [ N, L ]);
      removeAttrs(W, L);
      if (h) {
    ...
    let st = contents([ nt, tt, Z, J, W ].find((t => t && !elementIsGenerated(t))));
    const et = E ? W : nt || tt;
